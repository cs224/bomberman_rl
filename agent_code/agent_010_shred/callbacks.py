...
def act(self):
    ...
    self.logger.debug('pQQs after action mask: {}'.format(pQQs))
    ...
    if r < epsilon:
        ...
        if r1 < epsilon1:
            v = 1.0
            equal_sample = np.full(len(cav.DFIDs.A_ONE_HOT),v,dtype=np.float64)
            equal_sample[0] = v/10
            equal_sample = equal_sample * action_mask
            self.logger.debug('selecting action based on equal sampling: r1 = {}, epsilon1 = {}'.format(r1,epsilon1))
            next_action, c, weights = choice_sample(self, equal_sample)
        else:
            self.logger.debug('selecting action based on sampling: r1 = {}, epsilon1 = {}'.format(r1,epsilon1))
            next_action, c, weights = choice_sample(self, pQQs)
    else:
        self.logger.debug('selecting action based on max: r = {}, epsilon = {}'.format(r,epsilon))
        next_action, c, weights = choice_max(self, pQQs)
